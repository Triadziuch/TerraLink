...
void POWER_ConfigUnusedPinsAsAnalog(GPIO_TypeDef *ports[], uint16_t pin_masks[],
		uint8_t count) {
	for (uint8_t i = 0; i < count; ++i) {
		...
		GPIO_InitTypeDef init = { .Pin = pins, .Mode = GPIO_MODE_ANALOG, .Pull =
		GPIO_NOPULL };
...
void POWER_ConfigLoraPins(SX1278_hw_t *sx_hw, uint32_t mode, uint32_t pull) {
	...
	GPIO_InitTypeDef init = { .Mode = mode, .Pull = pull, .Speed =
	GPIO_SPEED_FREQ_LOW };
...
void POWER_RestoreLoraPins(SX1278_hw_t *sx_hw) {
	...
	GPIO_InitTypeDef init = { .Mode = GPIO_MODE_OUTPUT_PP, .Pull = GPIO_NOPULL,
...
void POWER_PrepareForSleep(SX1278_hw_t *sx_hw) {
	uint8_t unused_count = sizeof(UNUSED_GPIO_PIN_MASKS)
			/ sizeof(UNUSED_GPIO_PIN_MASKS[0]);
	POWER_ConfigUnusedPinsAsAnalog(UNUSED_GPIO_PORTS, UNUSED_GPIO_PIN_MASKS,
			unused_count);
